# Staging stack — definition only. Apply forbidden during PP-01B-IAC-01.
# No credentials, no remote backend, create_azure_resources must remain false.
...
module "core" {
  ...
  environment            = "staging"
  production_shaped      = var.production_shaped
...
check "staging_apply_forbidden" {
  assert {
    condition     = var.apply_authorized == false && var.create_azure_resources == false
    error_message = "Staging apply/create is forbidden during PP-01B-IAC-01."
  }
}
