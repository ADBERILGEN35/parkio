  ...
  - name: "critical"
    slack_configs:
      - api_url: '${SLACK_URL}'
        ...
        send_resolved: true
        title: '{{ if eq .Status "resolved" }}✅ Sorun çözüldü{{ else if eq .CommonLabels.severity "critical" }}🚨 Kritik{{ else }}⚠️ Uyarı{{ end }}: {{ if .CommonLabels.alertname }}{{ .CommonLabels.alertname }}{{ else }}bilinmeyen-uyarı{{ end }}'
        text: '{{ if .CommonLabels.environment }}Ortam: {{ .CommonLabels.environment }}{{ "\\n" }}{{ end }}{{ range .Alerts }}{{ if .Annotations.summary }}*{{ .Annotations.summary }}*{{ "\\n" }}{{ else }}*{{ .Labels.alertname | default "bilinmeyen-uyarı" }}*{{ "\\n" }}{{ end }}{{ if eq .Status "resolved" }}Durum: sorun çözüldü — koşul artık tetiklenmiyor.{{ "\\n" }}{{ else }}{{ if .Annotations.description }}Ne oldu: {{ .Annotations.description }}{{ "\\n" }}{{ else }}Ne oldu: Ek açıklama yok; etiketlere ve dashboard’a bakın.{{ "\\n" }}{{ end }}{{ end }}{{ if .Labels.service }}Servis: {{ .Labels.service }}{{ "\\n" }}{{ end }}{{ if .Labels.source_key }}Kaynak: {{ .Labels.source_key }}{{ "\\n" }}{{ end }}{{ if .Annotations.user_impact }}Kullanıcı etkisi: {{ .Annotations.user_impact }}{{ "\\n" }}{{ end }}{{ if .Annotations.value }}Değer: {{ .Annotations.value }}{{ "\\n" }}{{ end }}{{ if .Annotations.threshold }}Eşik: {{ .Annotations.threshold }}{{ "\\n" }}{{ end }}Başlangıç: {{ .StartsAt }}{{ "\\n" }}{{ if eq .Status "resolved" }}{{ else }}{{ if .Annotations.operator_action }}İlk işlem: {{ .Annotations.operator_action }}{{ "\\n" }}{{ else }}İlk işlem: runbook’u açın, etkilenen servisi/ kaynağı doğrulayın.{{ "\\n" }}{{ end }}{{ end }}{{ if .Annotations.runbook_url }}Runbook: {{ .Annotations.runbook_url }}{{ "\\n" }}{{ end }}{{ if .Annotations.dashboard_url }}Dashboard: {{ .Annotations.dashboard_url }}{{ "\\n" }}{{ end }}{{ "\\n" }}{{ end }}'
  - name: "warning"
    slack_configs:
      - api_url: '${SLACK_URL}'
        channel: '${SLACK_CHANNEL}'
