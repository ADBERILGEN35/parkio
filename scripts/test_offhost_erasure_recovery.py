...
def later_entries():
    return stamp_entries() + [{"authUserId": B, "erasedAt": T1}]
...
class OffhostErasureRecoveryTest(unittest.TestCase):
    ...
    def persist(self, store, entries, query_time, protocol=oe.PROTOCOL_LOCK):
        return oe.persist_complete_snapshot(
            store, entries, query_time, self.env, visibility_protocol=protocol
        )
    ...
    def test_account_present_in_backup_erased_afterward(self):
        store = oe.MemoryStore()
        result = self.persist(store, later_entries(), T1)
        self.assertEqual(result["coveredThrough"], T1)
        self.assertTrue(result["coverageAdvanced"])
        report = oe.recover(store, T1, stamp_entries())
        self.assertEqual(report["verdict"], "PASS")
        self.assertEqual(report["visibilityProtocol"], oe.PROTOCOL_LOCK)
    ...
    def test_unrelated_account_not_added(self):
        store = oe.MemoryStore()
        self.persist(store, stamp_entries(), T1)
        report = oe.recover(store, T1, stamp_entries())
        ids = {row["authUserId"] for row in report["merged"]}
    ...
    def test_remote_write_failure_does_not_advance_coverage(self):
        ...
        with self.assertRaises(oe.RemoteWriteError):
            self.persist(failing, later_entries(), T1)
    ...
    def test_interrupt_between_snapshot_and_seal(self):
        inner = oe.MemoryStore()
        failing = oe.FailingStore(inner, fail_times=1, fail_on="seals/")
        with self.assertRaises(oe.RemoteWriteError):
            self.persist(failing, later_entries(), T1)
        self.assertTrue(any(key.startswith("snapshots/") for key in inner.list()))
        self.assertFalse(any(key.startswith("seals/") for key in inner.list()))
        report = oe.recover(inner, T1, stamp_entries())
        self.assertEqual(report["verdict"], "BLOCKED")
        retried = oe.retry_pending(failing, self.env)
        self.assertTrue(retried["coverageAdvanced"])
        self.assertEqual(oe.recover(inner, T1, stamp_entries())["verdict"], "PASS")

    def test_query_time_without_lock_protocol_is_not_coverage(self):
        store = oe.MemoryStore()
        result = self.persist(store, later_entries(), T1, protocol=oe.PROTOCOL_ROWSET)
        self.assertFalse(result["coverageAdvanced"])
        self.assertIsNone(result["coveredThrough"])
        report = oe.recover(store, T1, stamp_entries())
        self.assertEqual(report["verdict"], "BLOCKED")
        self.assertIn("table-share-lock", report["reason"])
    ...
    def test_restore_refused_when_cutoff_uncovered(self):
        store = oe.MemoryStore()
        self.persist(store, later_entries(), T1)
    ...
    def test_persist_clock_is_not_coverage(self):
        ...
        self.assertEqual(covered["seal"]["visibilityProtocol"], oe.PROTOCOL_LOCK)
        self.assertNotEqual(covered["seal"]["coveredThrough"], CUTOFF)
        self.assertNotEqual(covered["seal"]["persistedAt"], CUTOFF)

    def test_newer_snapshot_must_keep_older_ids(self):
        store = oe.MemoryStore()
        self.persist(store, stamp_entries(), T1)
        with self.assertRaises(oe.OffhostError):
            self.persist(store, [{"authUserId": B, "erasedAt": T2}], T2)

    def test_stale_pending_after_newer_seal_is_discarded(self):
        store = oe.MemoryStore()
        self.persist(store, later_entries() + [{"authUserId": C, "erasedAt": T2}], T2)
        state = oe.load_state(store)
        state["pending"] = {
            "entries": stamp_entries(),
            "queryTime": T1,
            "visibilityProtocol": oe.PROTOCOL_LOCK,
        }
        oe.save_state(store, state)
        result = oe.retry_pending(store, self.env)
        self.assertFalse(result["retried"])
        self.assertEqual(result["discarded"], "stale-pending")
        report = oe.recover(store, T2, stamp_entries())
        self.assertEqual(report["verdict"], "PASS")
        self.assertEqual({row["authUserId"] for row in report["merged"]}, {A, B, C})

    def test_older_exporter_cannot_regress_coverage(self):
        store = oe.MemoryStore()
        self.persist(store, later_entries(), T2)
        with self.assertRaises(oe.StaleSnapshotError):
            self.persist(store, stamp_entries(), T1)
        report = oe.recover(store, T2, stamp_entries())
        self.assertEqual(report["verdict"], "PASS")
        self.assertEqual({row["authUserId"] for row in report["merged"]}, {A, B})

    def test_state_pointer_regression_keeps_newer_seal_ids(self):
        store = oe.MemoryStore()
        first = self.persist(store, stamp_entries(), T1)
        self.persist(store, later_entries(), T2)
        state = oe.load_state(store)
        state["lastSealSha256"] = first["sealSha256"]
        oe.save_state(store, state)
        report = oe.recover(store, T2, stamp_entries())
        self.assertEqual(report["verdict"], "PASS")
        self.assertEqual(report["coverageThrough"], T2)
        self.assertEqual({row["authUserId"] for row in report["merged"]}, {A, B})

    def test_public_result_strips_payload(self):
        report = {
            "verdict": "PASS",
            "merged": later_entries(),
            "entries": later_entries(),
            "recordCount": 2,
        }
        public = oe.public_result(report)
        self.assertNotIn("merged", public)
    ...
    def test_file_store_and_cli_roundtrip(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            ledger.write_text(json.dumps(later_entries()), encoding="utf-8")
            ...
            self.assertEqual(rowset.returncode, 0, rowset.stdout + rowset.stderr)
            self.assertNotIn(A, rowset.stdout)
            first = run(
                [sys.executable, str(script), "--from-ledger", str(ledger),
                 "--query-time", T1, "--visibility-protocol", "table-share-lock",
                 "--store-dir", str(store_dir)],
                env=env, capture_output=True, text=True, check=False,
            )
            self.assertEqual(first.returncode, 0, first.stdout + first.stderr)
            self.assertNotIn(A, first.stdout)
            ...
            self.assertNotIn(A, blocked.stdout)
            passed = run(
                [sys.executable, str(recover), "--store-dir", str(store_dir),
            ...
            self.assertNotIn(A, passed.stdout)
