...
class RateLimitKeyResolverTest {
    ...
    private final KeyResolver publicGeocodingResolver = new RateLimitConfig()
            .publicGeocodingIpKeyResolver(new ClientIpResolver(List.of("172.16.0.0/12")));
    ...
    @Test
    void publicGeocodingAlwaysUsesSpoofResistantResolvedIpEvenWithUserHeader() {
        MockServerHttpRequest request = MockServerHttpRequest
        ...
        String key = publicGeocodingResolver.resolve(MockServerWebExchange.from(request)).block();

        assertThat(key).isEqualTo("public-geocoding-ip:203.0.113.9");
        assertThat(key).doesNotStartWith("public-explore-ip:");
        assertThat(key).doesNotStartWith("user:");
    }

    @Test
    void publicGeocodingDoesNotTrustSpoofedForwardedForFromUntrustedPeer() {
        MockServerHttpRequest request = MockServerHttpRequest
                .get("/api/v1/public/geocoding/search")
                .remoteAddress(new InetSocketAddress("203.0.113.7", 5555))
        ...
        KeyResolver untrusted = new RateLimitConfig()
                .publicGeocodingIpKeyResolver(new ClientIpResolver(List.of()));
        String key = untrusted.resolve(MockServerWebExchange.from(request)).block();

        assertThat(key).isEqualTo("public-geocoding-ip:203.0.113.7");
    }
}
