...
/**
 * Maps domain {@link NotificationException}s, validation failures and infrastructure
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiError> handleNotification(NotificationException ex) {
        NotificationErrorCode code = ex.errorCode();
    ...
    /** Database constraint violation (e.g. a duplicate device-token race). */
    ...
    private static HttpStatus statusFor(NotificationErrorCode code) {
        return switch (code) {
            case MISSING_USER_ID -> HttpStatus.UNAUTHORIZED;
            case NOTIFICATION_NOT_FOUND, DEVICE_TOKEN_NOT_FOUND -> HttpStatus.NOT_FOUND;
        };
    }
}
