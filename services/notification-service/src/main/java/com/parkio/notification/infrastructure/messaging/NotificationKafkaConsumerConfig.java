...
/**
 * Kafka consumer wiring for the events notification-service subscribes to. The envelope
 * is read as a raw JSON string (deserialized by the listener), so a
 ...
 * backoff; exhausted/poison records are published to {@code parkio.dlt.notification} so a
...
public class NotificationKafkaConsumerConfig {
    ...
    public NotificationKafkaConsumerConfig(
    ...
    ConcurrentKafkaListenerContainerFactory<String, String> gamificationScoreKafkaListenerContainerFactory(
        ...
        DeadLetterPublishingRecoverer recoverer = new DeadLetterPublishingRecoverer(
                kafkaTemplate, (record, ex) -> new TopicPartition(DLT_NOTIFICATION, -1));
