...
/** Unit tests for the moderation.action → auth consumer: dispatch, ignore, ack. */
class ModerationActionsKafkaConsumerTest {
    ...
    private final AuthApplicationService service = mock(AuthApplicationService.class);
    ...
    @Test
    void ignoresUnknownFutureEventTypeButStillAcks() throws Exception {
        consumer.onMessage(record(UUID.randomUUID(), "SomeFutureModerationEvent"),
                "SomeFutureModerationEvent", ack);
        verify(service, never()).handleUserSuspended(any());
        verify(service, never()).handleUserRestored(any());
        verify(ack).acknowledge();
    }
    ...
    @Test
    void fallsBackToEnvelopeEventTypeWhenHeaderMissing() throws Exception {
        consumer.onMessage(record(UUID.randomUUID(), "UserSuspended"), null, ack);
        verify(service).handleUserSuspended(any());
        verify(ack).acknowledge();
    }
