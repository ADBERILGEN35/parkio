...
/**
 * Kafka consumer wiring for the events auth-service subscribes to
 * ({@code parkio.moderation.action} — account status sync). The envelope is read as a
 * raw JSON string (deserialized by the listener), so a {@code StringDeserializer} is
 * used here. Manual ack: the offset is committed only after a successful transaction.
 *
 * <p>Error handling (kafka-transport.md): transient failures are retried with a short
 * backoff; exhausted/poison records are published to {@code parkio.dlt.auth} so a single
...
public class AuthKafkaConsumerConfig {
    ...
    public AuthKafkaConsumerConfig(
    ...
    @Bean
    ConsumerFactory<String, String> authConsumerFactory() {
    ...
    @Bean
    ConcurrentKafkaListenerContainerFactory<String, String> authKafkaListenerContainerFactory(
            ConsumerFactory<String, String> authConsumerFactory,
        ...
        DeadLetterPublishingRecoverer recoverer = new DeadLetterPublishingRecoverer(
                kafkaTemplate, (record, ex) -> new TopicPartition(DLT_AUTH, -1));
