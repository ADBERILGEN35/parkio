...
import com.parkio.gamification.infrastructure.tracing.KafkaTraceContextSupport;
import org.springframework.scheduling.annotation.Scheduled;
import org.springframework.stereotype.Component;
import org.springframework.transaction.annotation.Transactional;

/**
 * In-process transactional-outbox relay for gamification-service. Polls unpublished
 * {@code outbox_events}, wraps each in the transport envelope and publishes it to
 * {@code parkio.gamification.score} keyed by the user id, then marks the row published —
 ...
 *
 * <p>Mirrors the {@code auth}/{@code parking} relays. All gamification score events share
 * one topic. Disable with {@code parkio.kafka.relay.enabled=false}.
 */
@Component
@ConditionalOnProperty(name = "parkio.kafka.relay.enabled", havingValue = "true", matchIfMissing = true)
public class GamificationOutboxRelay {

    private static final Logger log = LoggerFactory.getLogger(GamificationOutboxRelay.class);
