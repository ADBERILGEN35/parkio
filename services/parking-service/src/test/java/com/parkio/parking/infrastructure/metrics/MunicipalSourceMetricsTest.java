...
class MunicipalSourceMetricsTest {
    ...
    void recordsBoundedLabelsAndEmitsRecoveryWithoutExceptionText() {
        ...
        when(healthService.izumSnapshot())
                .thenReturn(snapshot(failing))
                .thenReturn(snapshot(failing))
                .thenReturn(snapshot(recovered));

        MunicipalSourceMetrics metrics = new MunicipalSourceMetrics(registry, healthService);
        ...
        assertThat(joined).doesNotContain("Read timed out")
                .doesNotContain("openapi.izmir")
                .doesNotContain("ignored");
    }

    private static MunicipalSourceHealthService.Snapshot snapshot(MunicipalSourceSlaPolicy.Evaluation evaluation) {
