...
@Component
public class IzumParkingClient {
    private final RestClient client;
    private final MunicipalSourceProperties.Izum config;

    public IzumParkingClient(RestClient.Builder builder, MunicipalSourceProperties properties) {
        this.config = properties.getIzum();
    ...
    public JsonNode fetch() {
        ...
        while (true) {
            try {
                ...
                throw ex; // 4xx is never retried
            } catch (RestClientException ex) {
                if (attempt++ >= config.getMaxRetries() || !transientFailure(ex)) throw ex;
                try {
    ...
    private static boolean transientFailure(RestClientException ex) {
        ...
        return true; // connection failures are transient
    }
}
