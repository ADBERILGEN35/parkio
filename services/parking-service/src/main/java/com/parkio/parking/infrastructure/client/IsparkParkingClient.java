...
/**
 * Read-only İSPARK facility list client. Base URL and path come from configuration only
 * (no user-controlled URLs). Does not call ParkDetay (list-only v1 strategy).
 */
@Component
public class IsparkParkingClient {
    private final RestClient client;
    private final MunicipalSourceProperties.Ispark config;

    public IsparkParkingClient(RestClient.Builder builder, MunicipalSourceProperties properties) {
        this.config = properties.getIspark();
