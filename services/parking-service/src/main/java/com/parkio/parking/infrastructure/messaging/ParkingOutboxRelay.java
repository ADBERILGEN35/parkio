...
import com.parkio.parking.infrastructure.config.KafkaTopicsConfig;
...
import com.parkio.parking.infrastructure.tracing.KafkaTraceContextSupport;
 ...
 *
 * <p>Publishing is <em>pipelined</em>: every send in a batch is dispatched first, then the
 * acks are awaited together under a single deadline, so the batch settles in roughly one
 * broker round-trip rather than {@code batchSize} serial ones. Kafka network I/O therefore
...
public class ParkingOutboxRelay {

    private static final Logger log = LoggerFactory.getLogger(ParkingOutboxRelay.class);
    ...
    public ParkingOutboxRelay(OutboxEventJpaRepository outbox,
