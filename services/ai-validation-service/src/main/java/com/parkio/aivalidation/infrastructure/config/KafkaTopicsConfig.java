...
public class KafkaTopicsConfig {
    ...
    @Bean
    NewTopic aiValidationResultTopic() {
        // Hot topic (advisory results; consumed by moderation, parking, analytics).
        return topic(AIVALIDATION_RESULT, 6, Duration.ofDays(7));
    }

    @Bean
    NewTopic aiValidationDltTopic() {
        return topic(DLT_AIVALIDATION, 3, Duration.ofDays(14));
    }
