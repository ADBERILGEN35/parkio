...
/**
 * In-process transactional-outbox relay for user-service. Polls unpublished
 * {@code outbox_events}, wraps each in the transport envelope and publishes it to
 * {@code parkio.user.profile} keyed by the authUserId, then marks the row published —
 ...
 *
 * <p>Mirrors {@code parking-service}'s relay. Disable with
...
public class UserOutboxRelay {
    ...
    private static final Logger log = LoggerFactory.getLogger(UserOutboxRelay.class);
    ...
    public UserOutboxRelay(OutboxEventJpaRepository outbox,
