...
/** Unit tests for the user outbox relay: envelope/headers/key/topic, publish-then-mark, DLQ. */
class UserOutboxRelayTest {
    ...
    private final UserOutboxRelay relay =
            new UserOutboxRelay(outbox, kafkaTemplate, objectMapper, new SimpleMeterRegistry(), 100, 5000L, 3);
    ...
    void continuesPublishingLaterRowsAfterOnePoisonRow() {
        OutboxEventEntity poison = profileCreatedRow(UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID());
        OutboxEventEntity healthy = profileCreatedRow(UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID());
    ...
    void publishesEveryPendingRowInOnePoll() {
        OutboxEventEntity a = profileCreatedRow(UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID());
        OutboxEventEntity b = profileCreatedRow(UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID());
